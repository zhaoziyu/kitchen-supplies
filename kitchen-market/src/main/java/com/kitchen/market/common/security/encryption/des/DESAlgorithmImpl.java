 ...
 * @date 2017-09-07
 */
class DESAlgorithmImpl {
    public static void encryptFile(String srcFile, String goalFile, byte[] key, DESAlgorithmType algorithm, DESWorkingType workingMode, DESPaddingType padding) throws GeneralSecurityException, IOException {
        operateFile(srcFile, goalFile, Cipher.ENCRYPT_MODE, key, algorithm.getName(), workingMode.getName(), padding.getName());
    }
    public static void decryptFile(String srcFile, String goalFile, byte[] key, DESAlgorithmType algorithm, DESWorkingType workingMode, DESPaddingType padding) throws GeneralSecurityException, IOException {
        operateFile(srcFile, goalFile, Cipher.DECRYPT_MODE, key, algorithm.getName(), workingMode.getName(), padding.getName());
    }

    public static byte[] encryptFile(byte[] data, byte[] key, DESAlgorithmType algorithm, DESWorkingType workingMode, DESPaddingType padding) throws GeneralSecurityException, IOException {
        return operateFile(data, Cipher.ENCRYPT_MODE, key, algorithm.getName(), workingMode.getName(), padding.getName());
    }
    public static byte[] decryptFile(byte[] data, byte[] key, DESAlgorithmType algorithm, DESWorkingType workingMode, DESPaddingType padding) throws GeneralSecurityException, IOException {
        return operateFile(data, Cipher.DECRYPT_MODE, key, algorithm.getName(), workingMode.getName(), padding.getName());
    }

    public static byte[] encryptText(byte[] data, byte[] key, DESAlgorithmType algorithm, DESWorkingType workingMode, DESPaddingType padding) {
        return operateText(Cipher.ENCRYPT_MODE, data, key, algorithm.getName(), workingMode.getName(), padding.getName());
    }
    public static byte[] decryptText(byte[] data, byte[] key, DESAlgorithmType algorithm, DESWorkingType workingMode, DESPaddingType padding) {
    ...
    private static Cipher createCipher(int mode, byte[] key, String algorithm, String workingMode, String padding) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidAlgorithmParameterException, InvalidKeyException {
        ...
        if (!DESWorkingType.ECB.getName().equalsIgnoreCase(workingMode)) {
            // 非ECB工作模式都需要传入向量
    ...
    private static byte[] getInvalidKey(byte[] key, String algorithm) {
        int keySize = 0;
        if (DESAlgorithmType.DES.getName().equals(algorithm)) {
            keySize = 8;// DES算法需要8位密钥
        } else if(DESAlgorithmType.DESede.getName().equals(algorithm)) {
            keySize = 24;// 3DES算法需要24位密钥
        }
    ...
    private static SecretKey getSecretKeyByGenerator(String algorithm, byte[] keyBytes) throws NoSuchAlgorithmException {
        ...
        int keySize = 0;
        if (DESAlgorithmType.DES.getName().equals(algorithm)) {
            keySize = 56;// DES算法需要8位密钥
        } else if(DESAlgorithmType.DESede.getName().equals(algorithm)) {
            keySize = 168;// 3DES算法需要24位密钥
        }

        SecureRandom random = SecureRandom.getInstance(DESAlgorithmType.SHA1PRNG.getName());
