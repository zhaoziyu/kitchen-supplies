...
import com.kitchen.market.common.security.sign.cer.KitSignTextByCertificate;
 ...
 */
@Ignore
public class TestTextSignByCertificate {
    ...
    @Test
    public void testTextSignV1() throws Exception {
        System.err.println("私钥签名——公钥验签");
        String data = "测试内容测试内容测试内容测试内容测试内容测试内容测试内容";
        System.out.println("待签名数据：" + data);
        long start = System.currentTimeMillis();
        String sign = KitSignTextByCertificate.signToBase64(data, keyStorePath, alias, password);
        long end = System.currentTimeMillis();
        System.err.println("根据KeyStore生成签名：" + sign + "    用时：" + (end - start) + "ms");

        start = System.currentTimeMillis();
        boolean result = KitSignTextByCertificate.verifyBase64Sign(data, sign, certificatePath);
        end = System.currentTimeMillis();
        System.err.println("使用Certificate校验签名结果：" + result + "    用时：" + (end - start) + "ms");
    }
    @Test
    public void testTextSignV2() throws Exception {
        System.err.println("私钥签名——公钥验签");
        String data = "测试内容测试内容测试内容测试内容测试内容测试内容测试内容";
        System.out.println("待签名数据：" + data);
        long start = System.currentTimeMillis();
        String sign = KitSignTextByCertificate.signToBase64(data, keyStore, alias, password);
        long end = System.currentTimeMillis();
        System.err.println("根据Certificate生成签名：" + sign + "    用时：" + (end - start) + "ms");

        start = System.currentTimeMillis();
        boolean result = KitSignTextByCertificate.verifyBase64Sign(data, sign, certificate);
        end = System.currentTimeMillis();
